...
| **AI04_DEPLOY_NOTES.md** (deploy/) | 2026-09-01 ai04 deploy runbook: compose, 13 startup defects, §8 revalidation, §9 real-stack upgrade @ a6e23e3, §10 relay live on ai04 (WSS E2E + admin mTLS green) | Before ANY compose/deploy change or shared-host deploy |
...
| **openspec/specs/cloud-control** | P3 RMM parity: hybrid MSP+per-org credentials, compute+storage+network inventory, tag drift, cost telemetry, auto-enrollment — DRAFT, plan at `docs/superpowers/plans/2026-09-02-cloud-control.md` | When designing/implementing cloud integration |
| **openspec/specs/eve-monitoring** | P3 RMM parity: dual host-agent + cluster-API integration (Proxmox/libvirt/vSphere), hypervisor-as-tag, HA/storage events — DRAFT, plan at `docs/superpowers/plans/2026-09-02-eve-monitoring.md` | When designing/implementing hypervisor integration |
| **openspec/specs/active-security** | P3 RMM parity: EDR ingest via webhook+pull (CrowdStrike/Defender/SentinelOne), SIEM forwarding (Splunk/ELK), burst handling, dedup — DRAFT, plan at `docs/superpowers/plans/2026-09-02-active-security.md` | When designing/implementing EDR/SIEM integration |
| **openspec/specs/power-monitoring** | P3 RMM parity: SNMP UPS check, agent battery check, power-event alert rules — DRAFT, plan at `docs/superpowers/plans/2026-09-02-power-monitoring.md` | When designing/implementing power monitoring |

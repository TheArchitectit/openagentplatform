...
| **AI04_DEPLOY_NOTES.md** (deploy/) | 2026-09-07 ai04 deploy runbook: §10 relay live on ai04 (WSS E2E + admin mTLS green + durable state), §11 P3 RMM parity redeploy @ a3fca20 (migrations v5→v16, all four domains live) | Before ANY compose/deploy change or shared-host deploy |
...
| **openspec/specs/cloud-control** | P3 RMM parity: hybrid MSP+per-org credentials, compute+storage+network inventory, tag drift, cost telemetry, auto-enrollment — SHIPPED `7ec4b69`; plan at `docs/superpowers/plans/2026-09-02-cloud-control.md` | When reviewing cloud integration |
| **openspec/specs/eve-monitoring** | P3 RMM parity: dual host-agent + cluster-API integration (Proxmox/libvirt/vSphere), hypervisor-as-tag, HA/storage events — SHIPPED `ae8ead6`; plan at `docs/superpowers/plans/2026-09-02-eve-monitoring.md` | When reviewing hypervisor integration |
| **openspec/specs/active-security** | P3 RMM parity: EDR ingest via webhook+pull (CrowdStrike/Defender/SentinelOne), SIEM forwarding (Splunk/ELK), burst handling, dedup — SHIPPED `1897f78`; plan at `docs/superpowers/plans/2026-09-02-active-security.md` | When reviewing EDR/SIEM integration |
| **openspec/specs/power-monitoring** | P3 RMM parity: SNMP UPS check, agent battery check, power-event alert rules — SHIPPED `1854907`; plan at `docs/superpowers/plans/2026-09-02-power-monitoring.md` | When reviewing power monitoring |
| **openspec/specs/phase7-test-coverage** | Phase 7: coverage gate (≥90% lines, ≥85% branches) + quality gate (go vet, staticcheck, gofmt); measured 21.0% overall 2026-09-07; priority tiers P1–P4; plan at `docs/superpowers/plans/2026-09-07-phase7-test-coverage.md` | When scoping or executing Phase 7 test coverage work |

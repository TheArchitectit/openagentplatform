    ...
    <div className="space-y-5" aria-busy={isLoading}>
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-white">A2A Dashboard</h1>
      ...
      {isLoading ? (
        ...
        <div role="list" aria-label="Adapter list" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filtered.map((a) => (
...
function AdapterCard({ adapter }: { adapter: A2AAdapter }) {
  ...
  return (
    <Link
      ...
      className="block rounded-lg border border-slate-800 bg-slate-900 p-4 hover:border-slate-700 hover:bg-slate-900 transition-colors"
    >
      <div className="flex items-start justify-between">
        <div className="flex items-center gap-2">
          <div className="h-9 w-9 rounded-md bg-slate-800 border border-slate-700 flex items-center justify-center text-blue-400" aria-hidden="true">
            {adapter.icon ? (
          ...
          <div>
            ...
            <div className="text-xs text-gray-400">v{adapter.version}</div>
          </div>
        </div>
        <span className={`h-2.5 w-2.5 rounded-full ${dot} flex-shrink-0 mt-1`} title={adapter.health} />
      </div>
      {adapter.description && (
        <p className="mt-2 text-xs text-gray-300 line-clamp-2">{adapter.description}</p>
      )}
      <div className="mt-3 flex flex-wrap gap-1">
        {adapter.skills?.slice(0, 4).map((s) => (
          <span
            key={s.name}
            className="text-[10px] uppercase tracking-wider px-1.5 py-0.5 rounded bg-slate-800 text-gray-300"
          >
            {s.name}
          </span>
        ))}

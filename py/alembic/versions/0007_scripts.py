...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "supported_platforms", JSONB, nullable=False, server_default=sa.text("'[]'::jsonb")
        ),
    ...
    op.create_index(
        "ix_script_definitions_org_runtime", "script_definitions", ["org_id", "runtime"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "script_id",
            UUID(as_uuid=True),
            sa.ForeignKey("script_definitions.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column(
            "agent_id",

...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "check_type IN ('ping', 'cpu', 'memory', 'disk', 'service', 'script', "
            "'event_log', 'process', 'wmi', 'custom')",
    ...
    op.create_index(
        "ix_check_definitions_org_template", "check_definitions", ["org_id", "is_template"]
    )
    op.create_index(
        "ix_check_definitions_org_status", "check_definitions", ["org_id", "last_status"]
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "check_id",
            UUID(as_uuid=True),
            sa.ForeignKey("check_definitions.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "check_assignment_id",
            UUID(as_uuid=True),
            sa.ForeignKey("check_assignments.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "agent_id",
    ...
    op.create_index(
        "ix_check_results_agent_check_time",
        "check_results",
        ["agent_id", "check_id", sa.text("execution_start DESC")],
    )
    op.create_index(
        "ix_check_results_org_status_time",
        "check_results",
        ["org_id", "status", sa.text("execution_start DESC")],
    )

...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "check_id",
            UUID(as_uuid=True),
            sa.ForeignKey("check_definitions.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column(
            "agent_id",
    ...
    op.create_index(
        "ix_alert_state_org_state_fired",
        "alert_state_machine",
        ["org_id", "state", sa.text("fired_at DESC")],
    )
    op.create_index(
        "ix_alert_state_org_severity_fired",
        "alert_state_machine",
        ["org_id", "severity", sa.text("fired_at DESC")],
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "alert_id",
            UUID(as_uuid=True),
            sa.ForeignKey("alert_state_machine.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "channel_id",

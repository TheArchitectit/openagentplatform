...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "win_update_policy", JSONB, nullable=False, server_default=sa.text("'{}'::jsonb")
        ),
    ...
    op.create_index(
        "ix_policy_definitions_org_priority", "policy_definitions", ["org_id", "priority"]
    )
    op.create_index(
        "ix_policy_definitions_org_active", "policy_definitions", ["org_id", "is_active"]
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "(client_id IS NOT NULL)::int + (site_id IS NOT NULL)::int + "

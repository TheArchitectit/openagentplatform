...
|------|----------|----------|
| OpenSpec P3 RMM parity gaps are now shipped — cloud control plane, hypervisor/EVE monitoring, active security/EDR, power/UPS monitoring. All four specs drafted, all four implementation plans executed; `f31e6a6..104f35c`. Known follow-ups: libvirt client needs C libvirt headers (build constraint). | Medium | openspec/specs/cloud-control; openspec/specs/eve-monitoring; openspec/specs/active-security; openspec/specs/power-monitoring |
| Phase 7 (test coverage + quality gate) is DRAFT. Coverage measured at 21.0% overall (2026-09-07); target ≥90% lines, ≥85% branches per MASTER_IMPLEMENTATION_PLAN.md §1.3. Spec and 7-task plan authored at openspec/specs/phase7-test-coverage/spec.md. Priority tiers: P1 (api/alerts/patches), P2 (policy/remote/relay), P3 (agent/scheduled/billing), P4 (cmd). | High | openspec/specs/phase7-test-coverage/spec.md; docs/superpowers/plans/2026-09-07-phase7-test-coverage.md |
| Managed A2A relay is deployed as a live compose service (`9472c24`, ai04 verified 2026-09-01: WSS two-leg rendezvous + admin mTLS green) with opt-in durable state shipped (`6715324`): migration 005, Postgres store, restart-survival proven live on ai04 (ledger + billing meters survive restart; live legs drop by design). Persistence deployed + verified `6715324`/`1a0cd8b` (healthcheck fix). | Medium | openspec/specs/a2a-relay/spec.md §8; deploy/AI04_DEPLOY_NOTES.md §10 |
...
| Guardrails pattern gate is blocking and clean as of 2026-09-15: 0 blocking violations, ~5300 advisory warnings. Largest contributor is OAP-008 ("function call result not captured" — most of its matches are `t.Error(...)`/`.Close()` lines whose real handling lives in `defer`/`if err` patterns the line-level regex cannot see). All blocking severity rules skip test files, comment-only lines, `exclude_glob` matches, `forbidden_context` lines, and `// guardrails-allow <id>:` annotations, matching `guardrails-scan.mjs`. Warnings are advisory; do not let them creep into blocking severity without a triage pass. | Medium | `.guardrails/prevention-rules/pattern-rules.json` (v4.4.0); `scripts/pattern-scan.sh`; `.github/workflows/guardrails-scan.yml` |

...
|------|----------|----------|
| OpenSpec P3 RMM parity gaps are now shipped — cloud control plane, hypervisor/EVE monitoring, active security/EDR, power/UPS monitoring. All four specs drafted, all four implementation plans executed; `f31e6a6..eca38e0`. Known follow-ups: libvirt client needs C libvirt headers (build constraint), EDR webhook HMAC validation is a TODO follow-up (verification logic exists, per-integration secret lookup is wired but the API handler path needs the integration store to expose GetByProvider). | Medium | openspec/specs/cloud-control; openspec/specs/eve-monitoring; openspec/specs/active-security; openspec/specs/power-monitoring |

...
func TestSplitDots(t *testing.T) {
	tests := []struct {
		...
		{"a.b", []string{"a", "b"}},
		{"a.b.c", []string{"a", "b", "c"}},
		{"oap.agents.agent-1.heartbeat", []string{"oap", "agents", "agent-1", "heartbeat"}},
	...
	for _, tt := range tests {
		t.Run(tt.input, func(t *testing.T) {
			...
			if len(got) != len(tt.want) {
				t.Fatalf("splitDots(%q) returned %d parts, want %d", tt.input, len(got), len(tt.want))
			}
			for i := range got {
				if got[i] != tt.want[i] {
...
func TestJoinDots(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.want, func(t *testing.T) {
			got := joinDots(tt.parts)
			if got != tt.want {
				t.Errorf("joinDots(%v) = %q, want %q", tt.parts, got, tt.want)
			}
		})
	}
}

func TestSplitDotsJoinDots_RoundTrip(t *testing.T) {
	inputs := []string{
		"oap.agents.agent-1.heartbeat",
		"a.b.c.d",
		"single",
		"",
	}
	for _, s := range inputs {
		parts := splitDots(s)
		joined := joinDots(parts)
		if joined != s {
			t.Errorf("roundtrip failed: %q -> split -> join = %q", s, joined)
		}
	}
}

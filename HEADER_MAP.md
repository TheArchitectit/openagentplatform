...
| cloud-control | SHIPPED | spec + 7-task plan, implementation landed `f31e6a6..7ec4b69`; build clean; AWS/Azure/GCP client SDKs use real `secrets.Resolver` signatures, drift detection wired, auto-enrollment creates virtual agents; cost-fetch SDK calls landed `eca38e0` (AWS Cost Explorer, Azure Cost Management, GCP explicit not-implemented) |
...
| phase7-test-coverage | DRAFT | coverage gate (≥90% lines, ≥85% branches) + quality gate; measured 21.0% overall 2026-09-07; spec + 7-task plan; priority tiers P1 (api/alerts/patches) → P2 (policy/remote/relay) → P3 (agent/scheduled/billing) → P4 (cmd) |
